...
class Generator extends \yii\gii\Generator
{
    ...
    public function getViewPath()
    {
        if (empty($this->viewPath)) {
            return Yii::getAlias('@frontend/views/' . $this->getControllerID());
        }
    ...
    public function generateActiveField($attribute)
    {
        ...
        if (preg_match('#date#',$attribute) || preg_match('#dob#',$attribute)){
            return "\$form->field(\$model, '$attribute')->widget(\dosamigos\datepicker\DatePicker::className(),[
        ...
        if (preg_match('#.*_id$#',$column->name)){
            ...
            try{
                new ReflectionMethod($this->modelClass,'get'.$name);
                ...
                if ($schema->getColumn("name")!=null){
                    return "\$form->field(\$model, '$attribute')->widget(\kartik\select2\Select2::className(),"
                          ."             ['data'=>\yii\helpers\ArrayHelper::map(\common\models\\".$name."::find()->all(),'id','name'),"
                ...
                try{
                    ...
                    if ($schema->getColumn("name")!=null){
                        return "\$form->field(\$model, '$attribute')->widget(\kartik\select2\Select2::className(),"
                            ."             ['data'=>\yii\helpers\ArrayHelper::map(\common\models\\".$name."s::find()->all(),'id','name'),"
                            ."             'options'=>['placeholder'=>'--Select--']"
                            ."])";
                    }
